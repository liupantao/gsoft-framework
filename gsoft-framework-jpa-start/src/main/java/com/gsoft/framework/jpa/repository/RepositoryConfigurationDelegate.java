...
public class RepositoryConfigurationDelegate {
	...
	private final Log logger = LogFactory.getLog(RepositoryConfigurationDelegate.class);
	...
	public List<BeanComponentDefinition> registerRepositoriesIn(BeanDefinitionRegistry registry,
		...
		for (RepositoryConfiguration<? extends RepositoryConfigurationSource> configuration : extension
			...
			if(beanNameSet.contains(beanName)){
				int i =0;
				String newBeanName = beanName + i;
				while(beanNameSet.contains(newBeanName)){
					newBeanName = beanName + i;
					i++;
				}
				beanName = newBeanName;
				logger.warn("存在相同名称的JpaDao,自动添加别名:"+beanDefinition.getBeanClassName()+"-"+beanName);
			}
