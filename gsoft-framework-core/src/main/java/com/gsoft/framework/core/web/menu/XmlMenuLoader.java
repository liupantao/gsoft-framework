...
public class XmlMenuLoader {
	...
	public HtmlTreeNode load(String resourcePath) {
		...
		if (resource != null) {
			...
			if (doc != null) {
				return TreeUtils.listToHtmlTree(getXmlMenus(doc), null, resourcePath, new TreeNodeConfig() {
					@Override
	...
	public List<IMenu> getMenus(String menuResource) {
		Resource resource = resourceLoader.getResource(menuResource);
		if (resource != null) {
			Document doc = parseDocument(resource);
	...
	private List<IMenu> getXmlMenus(Document doc) {
		...
		if (doc != null) {
			...
			for (Element element : menuElements) {
				...
				if ("menu".equals(element.getParent().getName())) {
