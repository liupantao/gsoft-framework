...
public class LayoutProvider implements ApplicationContextAware {
	...
	public static String DECORATOR_PREFIX = "decorator-";
	...
	private AbstractLayout getLayout(ResourceLoader resourceLoader, String decorator) {
		...
		if (layout == null) {
			String detectedKey = DECORATOR_PREFIX + decorator;

			Document document = getLayoutDoc(resourceLoader, decorator);
			if ((this.decorators != null) && (this.decorators.containsKey(decorator))) {
				layout = createLayout(document, resourceLoader, decorator);
			} else if (this.detectedLayouts.containsKey(detectedKey)) {
				layout = (AbstractLayout) this.detectedLayouts.get(detectedKey);
				layout.initLayout(document, decorator);
			} else {
				layout = createLayout(document, resourceLoader, "horizontal");
			}
	...
	private AbstractLayout createLayout(Document document, ResourceLoader resourceLoader, String decorator) {
		String className = (String) this.decorators.get(decorator);
		...
		if (document != null) {
			...
			Constructor<?> constructor = ClassUtils.getConstructorIfAvailable(clazz, parameterTypes);
	...
	public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
		if (this.detectedLayouts == null)
			this.detectedLayouts = BeanFactoryUtils.beansOfTypeIncludingAncestors(applicationContext, AbstractLayout.class, true,
					false);
