package com.gsoft.framework.httpclientutil;
...
import com.gsoft.framework.httpclientutil.builder.HCB;
import com.gsoft.framework.httpclientutil.common.HttpConfig;
import com.gsoft.framework.httpclientutil.common.HttpMethods;
import com.gsoft.framework.httpclientutil.common.Utils;
import com.gsoft.framework.httpclientutil.exception.HttpProcessException;
	...
	private static HttpResponse execute(HttpConfig config) throws HttpProcessException {
		...
		try {
			...
			if(config.getConfig() != null) {
				request.setConfig(config.getConfig());
			}
			...
			// 执行请求操作，并拿到结果（同步阻塞）
			resp = (config.context() == null) ? config.client().execute(request) : config.client().execute(request,
					config.context());
